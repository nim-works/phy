...
proc check_continuation*(tree: Tree, n: var NodeIndex, err: var Error): bool =
  ...
  matchTree "(Continuation (Params) (Locals ...) <stmt>* <exit>)", Continuation, match:
    ...
    if num == len or not result: break match
    inc num
    matchTree "(Locals <local>*)", Locals, match:
      while num < len:
        if not check_local(tree, n, err): break
        inc num
    if num == len or not result: break match
    inc num
    while num < len:
    ...
    inc num
  if result: return true
  matchTree "(Except <local> (Locals ...) <stmt>* <exit>)", Except, match:
    if num == len or not check_local(tree, n, err): break match
    inc num
    matchTree "(Locals <local>*)", Locals, match:
      while num < len:
