...
proc genConvOp(c; tree; n: NodeIndex, op: string): Value =
  ## Emits the LLVM conversion operation `op` for the IL operation at `n`.
  let
    (dtyp, styp, v) = triplet(tree, n)
    ltyp = shortTy(tree, dtyp)
    val  = c.genExpr(tree, v, shortTy(tree, styp))
  c.emitOp(ltyp, fmt"{op} {val} to {ltyp}")
...
proc genExpr(c; tree; val: NodeIndex, typ: LLVMType): Value =
  ...
  of Nil:
    Value(kind: vkConst, val: "null", typ: PtrType)
  of Global:
    global("@g" & $tree[val].id, PtrType)
  of Copy:
    ...
    of Global:
      # FIXME: this is wrong for constants, which require a load
  ...
  of Conv:
    ...
    # NOTE: the conversion from float-to-int (and vice versa) is not well-
    # defined at the moment, invoking undefined behaviour at the LLVM side
    case dst.kind
    of Int:
    ...
    of Float:
      ...
      of Float:
        unreachable()
  of Promote:
    c.genConvOp(tree, val, "fpext")
  of Demote:
    c.genConvOp(tree, val, "fptrunc")
  of Zext:
    c.genConvOp(tree, val, "zext")
  of Sext:
    c.genConvOp(tree, val, "sext")
  of Trunc:
    c.genConvOp(tree, val, "trunc")
  of Call:
...
proc llvmStr(x: string): string =
  ## Creates an LLVM string literal from `x`.
  result.add '\"'
  for c in x.items:
    case c
    of '\0'..'\31', '\127'..'\255':
...
proc translate*(module: PackedTree[NodeKind]): string =
  ...
  for i, def in module.pairs(globals):
    ...
    case module[val].kind
    of Data:
      let align = module.getUInt(module.child(val, 0))
      if module[val, 1].kind == StringVal:
        let init = module.getString(module.child(val, 1))
        c.code.add &"@g{i} = private global [{init.len} x i8] c{llvmStr(init)}, align {align}\n"
      else:
        let size = module.getUInt(module.child(val, 1))
        c.code.add &"@g{i} = private global [{size} x i8] zeroinitializer, align {align}\n"
    else:
      # a non-data initializer can currently only mean that it's a constant
      let init = c.genExpr(module, val, shortTy(module, typ))
      c.code.add &"@g{i} = private constant {init}\n"
