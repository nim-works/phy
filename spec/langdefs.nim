...
proc semConstrDef(c; n: NimNode, typ: Type): Type =
  ...
  of nnkIdent, nnkAccQuoted:
    ...
    # TODO: ^^ don't
    let pat = Node(kind: nkSymbol, sym: name(n))
    result = Type(kind: tkPat, pat: tree(nkConstr, pat))
    c.addConstrSym(result.pat[0].sym, result, n)
  of nnkCall:
